#!/bin/sh
# polyptic-agent — post-install hook (deb postinst / rpm post; one script for both).
#
...
if command -v systemctl >/dev/null 2>&1; then
  ...
  systemctl enable polyptic-agent-firstboot.service || true
fi
...
#    guaranteed `polyptic-agent setup`) is the completion path, and `sudo polyptic-agent setup`
#    always finishes it by hand.
if command -v polyptic-agent >/dev/null 2>&1; then
  if ! POLYPTIC_NONINTERACTIVE=1 polyptic-agent setup --no-pkg; then
    echo "polyptic-agent: deferred full provisioning to first boot (or run 'sudo polyptic-agent setup')." >&2
  fi
fi

echo "polyptic-agent installed."
echo "  1. Edit /etc/polyptic/agent.toml  (control-plane URL + bootstrap token)."
