#!/bin/sh
# polyptic-agent — pre-remove hook (deb prerm / rpm preun; one script for both).
#
...
if command -v systemctl >/dev/null 2>&1; then
  systemctl disable --now polyptic-agent-firstboot.service 2>/dev/null || true
fi
...
if command -v polyptic-agent >/dev/null 2>&1; then
  polyptic-agent teardown --no-pkg 2>/dev/null || true
fi
