...
## mTLS agent channel — reachability (POL-25/POL-134, POL-143/D120)
...
| `agentMtls.expose` / `.nodePort` | `nodePort` / `30843` | How the raw-TCP listener is published so boxes can reach it (POL-143/D120). `none` = BYO reachability + `publicUrl`. See the mTLS reachability section. |
