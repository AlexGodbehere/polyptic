...
/** The library row's sub-line: a playlist reads as its step count, a URL kind as its address. */
function rowSub(s: ContentSource): string {
  if (s.kind === "playlist") {
    const n = s.items?.length ?? 0;
    return `${kindLabel(s.kind)} · ${n} step${n === 1 ? "" : "s"}`;
  }
...
// ── playlists (POL-34) ─────────────────────────────────────────────────────────
// A playlist is authored as an ORDERED LIST of existing library sources with per-step timing, so its
// modal is a list builder rather than the URL form. Steps that are not videos need a hold time
// (defaulted to 15 s); a video left blank plays to the end before the playlist moves on.
const DEFAULT_ITEM_SECONDS = 15;
const plModalOpen = ref(false);
const plEditingId = ref<string | null>(null); // null = creating
const plName = ref("");
/** Draft steps keep the duration as TEXT so a video's blank ("until it ends") survives editing. */
const plItems = ref<{ sourceId: string; duration: string }[]>([]);
...
const stepSources = computed(() => sources.value.filter((s) => s.kind !== "playlist"));

const plModalTitle = computed(() => (plEditingId.value ? "Edit playlist" : "New playlist"));
const plSaveLabel = computed(() => (plEditingId.value ? "Save changes" : "Create playlist"));
...
function openAddPlaylist() {
  ...
  plName.value = "";
  plItems.value = [blankStep()];
  plError.value = null;
  plModalOpen.value = true;
}

function openEditPlaylist(s: ContentSource) {
  plEditingId.value = s.id;
  plName.value = s.name;
  plItems.value = (s.items ?? []).map((i) => ({
    sourceId: i.sourceId,
    duration: i.durationSeconds !== undefined ? String(i.durationSeconds) : "",
  }));
  if (plItems.value.length === 0) plItems.value = [blankStep()]; // emptied by source deletions
  plError.value = null;
  plModalOpen.value = true;
}

function closePlaylistModal() {
  plModalOpen.value = false;
  plSaving.value = false;
}

function addStep() {
  plItems.value.push(blankStep());
}
function removeStep(i: number) {
  plItems.value.splice(i, 1);
}
...
async function savePlaylist() {
  if (plSaving.value) return;
  plError.value = null;

  const items: { sourceId: string; durationSeconds?: number }[] = [];
  for (const [i, draft] of plItems.value.entries()) {
    if (!draft.sourceId) {
      plError.value = `Step ${i + 1}: pick a source.`;
      return;
    }
    const text = draft.duration.trim();
    if (!text) {
      if (stepKind(draft.sourceId) !== "video") {
        plError.value = `Step ${i + 1}: set how long it shows, in seconds.`;
        return;
      }
    ...
    if (!Number.isInteger(seconds) || seconds < 1 || seconds > 86400) {
      plError.value = `Step ${i + 1}: the duration must be 1–86400 whole seconds.`;
      return;
    }
  ...
  if (!parsed.success) {
    plError.value = parsed.error.issues[0]?.message ?? "Please check the fields.";
    return;
  }

  plSaving.value = true;
  const ok = plEditingId.value
    ? await store.updateSource(plEditingId.value, { name, items })
    : await store.createSource(parsed.data);
  plSaving.value = false;
  if (ok) {
    plModalOpen.value = false;
  } else {
    plError.value = "Couldn't save the playlist. Check the steps and try again.";
  }
}
</script>
...
<template>
  <div class="page">
    <div class="page-inner">
      ...
      <header class="head">
        ...
        <div class="head-actions">
          ...
          <button class="add-btn ghost compact" @click="openAddPlaylist">≣ New playlist</button>
    ...
    <!-- ── playlist modal (POL-34) ──────────────────────────────────────────── -->
    <div v-if="plModalOpen" class="scrim" @mousedown.self="closePlaylistModal">
      <div class="modal wide" role="dialog" aria-modal="true">
        <div class="modal-title">{{ plModalTitle }}</div>

        <label class="field-label">Name</label>
        <input v-model="plName" class="field" placeholder="e.g. Lobby rotation" />

        <label class="field-label">
          Steps <span class="optional">(played top to bottom, then repeats)</span>
        </label>
        <div class="steps">
          <div v-for="(item, i) in plItems" :key="i" class="step-row">
        ...
        <div v-if="plError" class="error">⚠ {{ plError }}</div>

        <div class="modal-actions">
          <button class="btn-secondary" @click="closePlaylistModal">Cancel</button>
          <button class="btn-primary" :disabled="plSaving" @click="savePlaylist">
            {{ plSaveLabel }}
          </button>
        </div>
      </div>
