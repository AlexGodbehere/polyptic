...
export interface MediaConfig {
   ...
   *  with no converter there is no deck to show, so a document upload is REFUSED, not accepted (D124). */
...
export function registerRestRoutes(
  ...
  fastify.post("/api/v1/media", async (request, reply) => {
    ...
    // has been converted, so storing it would create a library row that provably cannot paint (D124).
    if (docExt) {
    ...
    // means "it will show on a wall". A server with no toolchain accepts and warns (D123).
