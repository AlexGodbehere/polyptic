 ...
 * Backend selection.
 *
 * Explicit override via `POLYPTYCH_BACKEND` (dev-open | wayland-sway | x11-i3).
 ...
 * explicitly with POLYPTYCH_BACKEND once they're implemented.
 */
...
export function selectBackend(env: NodeJS.ProcessEnv = process.env): DisplayBackend {
  const forced = env.POLYPTYCH_BACKEND?.trim();
  if (forced) {
    switch (forced) {
      case "dev-open":
      ...
      default:
        throw new Error(
          `Unknown POLYPTYCH_BACKEND="${forced}" (expected: dev-open | wayland-sway | x11-i3)`,
        );
    }
  }
