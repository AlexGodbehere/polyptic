...
A self-built iPXE `.efi` from `deploy/build-ipxe.sh` is **unsigned**, so UEFI refuses it with **Secure Boot ON** — identically from a USB dongle, from the offloaded ESP entry, from PXE/TFTP, or from UEFI HTTP Boot. **Disable Secure Boot** in firmware (or move it to Setup Mode). This is the same single prerequisite for *both* dongle and offload modes. Signing iPXE via Microsoft's `shim` (to keep Secure Boot on) is future work the tokenless model deliberately avoids.
