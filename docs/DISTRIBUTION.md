...
| `PUBLIC_BASE_URL` | The ONE public origin (`scheme://host[:port]`) | **Set it in prod.** `https://` turns Secure cookies on automatically; a declared `http://` turns them off (browsers silently drop Secure cookies over http — login would never persist) with a loud boot banner (POL-70/D88). |
