...
**Delivery = `apt install polyptych-agent`** (a `.deb`), NOT a mandatory image (D26). Setup logic lives in the agent binary (`polyptych-agent setup`, distro-aware: apt/dnf/pacman → generic across systemd Linux); image + cloud-init/Ansible are optional wrappers. Start from **Ubuntu Server-minimal** (D27); the package wires **greetd** autologin → **sway** (Wayland; `x11-i3` fallback for NVIDIA, D9) → **systemd**-supervised agent + **Chromium-per-output** (`.deb` Chromium not the snap; `cog`/WPE fallback). **Make the `DisplayBackend`s real** — replace the Phase-1 sway/x11 stubs with swaymsg-IPC placement + Chromium launching. Crash/restore hardening (`Restart=always`, popup/`exit_type` suppression, no `swayidle`, `dpms on`). Config (control-plane URL + bootstrap token) via debconf or `/etc/polyptych/agent.toml` → it enrols (2b) → approve in the console.
